...
import MobilePreview from "../assets/images/illustration-phone-mockup.svg";
  ...
  const { loading, error, data } = useQuery(GET_USER_PROFILE, {
    onCompleted: (data) => {
      ...
      if (data?.users?.[0]?.links) {
        const transformedLinks = data.users[0].links.map(link => ({
          id: link.id,
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        <SelectTrigger className="h-12">
                            <SelectValue>
                            {/* {({ value }) => {
                              const PlatformIcon = platforms[value]?.icon || Github;
                              return (
                                <div className="flex items-center gap-2">
                                  <PlatformIcon className="h-5" />
                                    {platforms[value]?.name || "Select platform"}
                                </div>
                              );
                            }} */}
                          </SelectValue>
